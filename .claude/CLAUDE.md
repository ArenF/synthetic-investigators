...
`**[행동]**`, `**[시도]**`, `**[내면]**`, `**[OOC]**` 태그를 정규식으로 추출한다.
`[OOC]`는 game 모드에서 `[내면]` 대신 사용 — 동일한 `inner` 필드로 파싱됨.

```typescript
// [OOC]와 [내면] 모두 inner 필드로 파싱 (backward compat)
...
`thinkingTakeTurn`의 3단계 호출에 사용되는 함수들 (모두 mode + provider 파라미터 수용):

```
buildInnerStageInstruction(mode)            → Stage 1: [내면]/[OOC] 역할 설명 포함
buildAttemptStageInstruction(mode)          → Stage 2: [시도] 역할 설명 포함
buildActionStageInstruction(mode, provider) → Stage 3: [행동] 역할 + 형식 힌트 포함
...
wsReady: boolean
playMode: PlayMode
pendingAttempts: AttemptDeclared[]  // attempt_declared 수신 시 큐에 추가 (FIFO)
// enqueuePendingAttempt() / dequeuePendingAttempt()로 관리
// 같은 턴 여러 AI 시도 선언 시 순서대로 처리
```
...
5. **코드 정리**: `sanCheck()` 데드코드 제거, 판정 시스템 중복 로직 통합, `accumulatedContext` 재구성 개선
