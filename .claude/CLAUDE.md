...
server/
  ...
  players/
    ...
    claude-player.ts          Claude API chat() 구현
    gemini-player.ts
    openai-player.ts
    ollama-player.ts
...
### ClaudePlayer — chat() 구현

`chat()` 하나만 구현하며 별도 오버라이드 없음. `thinkingTakeTurn()`은 `BasePlayer` 기본 구현을 그대로 사용한다.
...
> **참고**: Extended Thinking 기능은 commit `214b9fe`에서 제거됨.
...
```typescript
screen: 'home' | 'session_setup' | 'game' | 'character_editor' | 'log_viewer' | 'scenario_list' | 'scenario_editor'
...
wsReady: boolean
playMode: PlayMode
pendingSetup: SessionSetupData | null  // 세션 생성 중 임시 상태
```
...
| `ScenarioList.tsx` | 저장된 시나리오 목록 표시 + 세션 재개 |
| `LogViewer.tsx` | 저장된 세션 로그 텍스트 조회 |
| `Layout.tsx` | 최소 레이아웃 래퍼 컴포넌트 |
