...
abstract class BasePlayer {
  ...
  async thinkingTakeTurn(ctx: TurnContext): Promise<TurnRecord>   // 3단계 사고 트리
}
```

### thinkingTakeTurn — 히스토리 관리 패턴

사고 트리의 3단계(내면→시도→행동)는 **스테이징 메시지**를 임시로 히스토리에 쌓은 뒤,
...
// 3단계 메시지 임시 push
this.history.push(...stage1...)
this.history.push(...stage2...)
this.history.push(...stage3...)
...
`thinkingTakeTurn()`을 오버라이드하여 3회 호출 대신 **1회 호출**로 사고 트리를 처리한다.
...
정규식으로 `**[행동]**`, `**[시도]**`, `**[내면]**` 태그를 추출한다.
세 태그가 어느 순서로 오든 동작하도록 lookahead로 구분한다.

```typescript
action: raw.match(/\*\*\[행동\]\*\*\s*([\s\S]*?)(?=\*\*\[시도\]\*\*|\*\*\[내면\]\*\*|$)/i)
// 태그가 없으면 raw.trim() 전체를 action으로 사용
action: actionMatch?.[1]?.trim() ?? raw.trim()
```

### 모델별 힌트 (getProviderHints)

시스템 프롬프트 끝에 추가되는 모델 전용 지침:

| 모델 | 주요 지침 |
|------|---------|
| Claude | GM 묘사를 따라 확장 가능하되 직접 묘사 생성 금지 |
| Gemini | `[행동]` + `[내면]` 필수 작성, 임의 시간 점프 금지 |
| OpenAI | GM 질문 먼저 답하고 행동 이어가기 |
| Ollama | 한국어로만 응답 |

### 사고 트리 단계 지시문

`thinkingTakeTurn`의 3단계 호출에 사용되는 함수들:

```
buildInnerStageInstruction()   → "[내면]만 작성하세요" — Stage 1
buildAttemptStageInstruction() → "[시도]만 작성하세요" — Stage 2 (이전 [내면] 히스토리 포함)
buildActionStageInstruction()  → "[행동]만 작성하세요" — Stage 3 (이전 [내면]+[시도] 포함)
buildThinkingTreeSystemSuffix() → Claude Extended Thinking용 system prompt 접미사
```
...
### detectSkillFromText — [시도] 텍스트에서 기술명 자동 감지

현재 목록 (32개): `격투, 사격, 회피, 흔적발견, 청취, 도서관이용, 역사, 오컬트, 설득, 위협, 매혹, 심리학, 응급처치, 운전, 은신, 잠입, 의학, 생물학, 화학, 물리학, 법률, 회계, ...`

**알려진 이슈**: DEVLOG에 CoC 7th 공식 기술 목록(약 50개)으로 전체 교체 예정.
...
| `attempt_declared` | AI가 [시도] 선언 → GM에게 팝업 |
...
### 알려진 버그: getCharacterStates의 아이템

```typescript
// 현재 (버그): 세션 중 변경된 아이템을 반영하지 않음
equipment: char.equipment?.items ?? []

// 수정 필요: 세션 상태의 currentItems 사용
...
wsReady: boolean
playMode: PlayMode
pendingAttempt: {...} | null   // attempt_declared 수신 시 설정 → AttemptReviewModal 표시
```
...
| `AttemptReviewModal.tsx` | AI [시도] 선언 감지 → GM 검토 팝업 |
...
1. **판정 플로우 개편**: `AttemptReviewModal`에 6단계 결과 에디터 통합, 판정 후 자동 AI 턴 실행 + `isJudgmentResponse: true` 플래그로 재귀 [시도] 차단
2. **CoC 7th 공식 기술 목록 교체**: 현재 32개 → 공식 약 50개. 변경 범위: `ActionRequestModal`, `AttemptReviewModal`, `judgment.ts detectSkillFromText()`, `CharacterEditor`, `types.ts SkillSet`
3. **기술값 0 숨김**: `ActionRequestModal`/`AttemptReviewModal` 드롭다운에서 해당 캐릭터의 0 기술값 항목 제외
4. **아이템 버그 수정**: `getCharacterStates()`에서 `char.equipment.items` → `s.currentItems` 교체
5. **턴 플로우 자동화**: 턴 순서 연동 자동 진행 옵션 (설계 미확정)
