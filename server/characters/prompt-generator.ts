...
/**
 * Stage 1: 내면 — 감정과 즉각적인 생각
 */
export function buildInnerStageInstruction(): string {
  return `**[내면]만** 작성하세요.
지금 이 순간 느끼는 감정, 직관, 즉각적인 생각을 솔직하게 표현하세요.
행동 계획이나 기술 판정은 포함하지 마세요. **[내면]** 태그로 시작하세요.`
}

/**
 * Stage 2: 시도 — 상황 직시 + 행동 판단 + 조언
 * 이전 [내면]이 히스토리에 있는 상태에서 호출됨
 */
export function buildAttemptStageInstruction(): string {
  return `위 [내면]을 바탕으로 **[시도]만** 작성하세요.
상황을 냉정하게 직시하고, 무엇을 해야 하는지 판단하며, 스스로에게 주는 조언 문장을 담으세요.
...
1단계 [내면]: 지금 느끼는 감정과 즉각적인 생각은?
2단계 [시도]: 상황을 어떻게 직시하는가? 무엇을 해야 하는가? 스스로에게 주는 조언은?
